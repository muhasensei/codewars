...
block for each letter (direction) and you know it takes you one minute to traverse one city
 block, so create a function that will return true if the walk the app gives you will take you 
...
function isValidWalk(walk) {
    let ns_pairs = 0, ew_pairs = 0;
    walk.map(letter => {
      (letter === 'n') ? ns_pairs++ :
        (letter === 's') ? ns_pairs-- :
          (letter === 'w') ? ew_pairs++ : ew_pairs--
    });
    return walk.length === 10 && !ns_pairs && !ew_pairs;
}
