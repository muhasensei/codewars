...
// замароченная (ибанутая) версия
function likes(names) {
    const len = names.length;
    const likes = ' likes this';
    const like = ' like this';
    const others = `${len - 2} others`;
    if(len > 0){
        return names[0] + (len > 1 ? (len > 2 ? ', ' : ' and ') + names[1] + (len > 2 ? ' and ' + (len > 3 ? others : names[2]) : '') + like : likes);
    }

    return 'no one' + like;
}

console.log(likes(['Muha', 'Evrei', 'Arman', 'Kurwa']));